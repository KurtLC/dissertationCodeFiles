...
class YeelightGenericLight(YeelightEntity, LightEntity):
    ...
    def rgb_color(self) -> tuple[int, int, int] | None:
        ...
        _LOGGER.info(f"CustomLog || YeelightComponents || YeelightGenericLight class || rgb_color method || Actual RGB value - Red: {red} ; Green: {green} ; Blue: {blue}")

        blue = (rgb >> 8) & 0xFF #modified from:   rgb & 0xFF
        green = (rgb >> 16) & 0xFF #modified from: (rgb >> 8) & 0xFF
        red = rgb & 0xFF #modified from:           (rgb >> 16) & 0xFF

        _LOGGER.info(f"CustomLog || YeelightComponents || YeelightGenericLight class || rgb_color method || Manipulated RGB value - Red: {red} ; Green: {green} ; Blue: {blue}")
    ...
    def _properties(self) -> dict:
        ...
        lastProp = (self._bulb.last_properties if self._bulb else {})
